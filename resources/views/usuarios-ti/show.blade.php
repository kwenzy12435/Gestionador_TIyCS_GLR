  ...
  <div class="card-header bg-white">
    <div class="row align-items-center">
      ...
      <div class="col-md-6 text-end">
        <div class="btn-group">
          <a href="{{ route('usuarios-ti.edit', $usuario) }}" class="btn btn-warning btn-sm">
    ...
    <div class="row g-4">
      <div class="col-md-6">
        <div class="border-bottom pb-3 mb-3">
          ...
          <p><strong>Puesto:</strong> {{ $usuario->puesto ?? '<span class="text-muted">—</span>' }}</p>
        </div>
        
        <div>
          <h6 class="fw-semibold text-primary mb-2"><i class="bi bi-telephone me-2"></i>Contacto</h6>
          <p><strong>Teléfono:</strong> {{ $usuario->telefono ?? '<span class="text-muted">—</span>' }}</p>
        </div>
      </div>
      ...
      <div class="col-md-6">
        <div class="border-bottom pb-3 mb-3">
          ...
          <p>
            <strong>Rol:</strong> 
            <span class="badge {{ $usuario->rol === 'ADMIN' ? 'bg-danger' : ($usuario->rol === 'AUXILIAR-TI' ? 'bg-info' : 'bg-secondary') }}">
              {{ $usuario->rol }}
            </span>
          </p>
