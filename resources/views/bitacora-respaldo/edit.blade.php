...
<div class="card p-4 shadow-sm">
  <form method="POST" action="{{ route('bitacora-respaldo.update', $bitacoraRespaldo) }}" novalidate>
    @csrf
    @method('PUT')
    ...
    <div class="row g-3">
      ...
      <div class="col-md-4">
        ...
        <input type="date" name="fecha_respaldo" id="fecha_respaldo"
               ...
               value="{{ old('fecha_respaldo', $bitacoraRespaldo->fecha_respaldo->format('Y-m-d')) }}" required>
        @error('fecha_respaldo') <div class="invalid-feedback">{{ $message }}</div> @enderror
      </div>
        ...
        <select name="usuario_ti_id" id="usuario_ti_id"
          ...
          @foreach($usuariosTi as $usuario)
            <option value="{{ $usuario->id }}"
              {{ old('usuario_ti_id',$bitacoraRespaldo->usuario_ti_id)==$usuario->id?'selected':'' }}>
      ...
      <div class="col-md-6">
        ...
        <input type="text" name="ubicacion_guardado" id="ubicacion_guardado"
               ...
               value="{{ old('ubicacion_guardado', $bitacoraRespaldo->ubicacion_guardado) }}" maxlength="255">
        @error('ubicacion_guardado') <div class="invalid-feedback">{{ $message }}</div> @enderror
      </div>
      ...
      <div class="col-12">
        ...
        @error('acciones_alternativas') <div class="invalid-feedback">{{ $message }}</div> @enderror
      </div>
    </div>
