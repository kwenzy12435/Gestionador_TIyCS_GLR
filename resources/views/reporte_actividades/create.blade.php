...
@section('title', 'Nuevo Reporte - Sistema de Gestión TI')
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center pb-2 mb-3 border-bottom">
                <h1 class="h2">Nuevo Reporte de Actividad</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('reporte_actividades.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('actividad') is-invalid @enderror" 
                                       id="actividad" name="actividad" value="{{ old('actividad') }}" required>
                                @error('actividad')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('naturaleza_id') is-invalid @enderror" id="naturaleza_id" name="naturaleza_id">
                                    ...
                                    @foreach($naturalezas as $naturaleza)
                                        <option value="{{ $naturaleza->id }}" {{ old('naturaleza_id') == $naturaleza->id ? 'selected' : '' }}>
                                            {{ $naturaleza->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('usuario_ti_id') is-invalid @enderror" id="usuario_ti_id" name="usuario_ti_id">
                                    ...
                                    @foreach($usuariosTi as $usuarioTi)
                                        <option value="{{ $usuarioTi->id }}" {{ old('usuario_ti_id') == $usuarioTi->id ? 'selected' : '' }}>
