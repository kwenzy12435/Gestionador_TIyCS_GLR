        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center pb-2 mb-3 border-bottom">
                ...
                <div>
                    ...
                    <a href="{{ route('usuarios-ti.edit', $usuarios_ti) }}" class="btn btn-warning">  <!-- CAMBIADO -->
                ...
                <div class="col-md-4">
                    <div class="card text-center">
                        <div class="card-body">
                            ...
                            <h4 class="card-title">{{ $usuarios_ti->nombres }} {{ $usuarios_ti->apellidos }}</h4>  <!-- CAMBIADO -->
                            <span class="badge bg-{{ $usuarios_ti->rol == 'ADMIN' ? 'danger' : ($usuarios_ti->rol == 'AUXILIAR-TI' ? 'warning' : 'primary') }} fs-6">  <!-- CAMBIADO -->
                                {{ $usuarios_ti->rol }}  <!-- CAMBIADO -->
                            </span>
                            <p class="text-muted mt-2">{{ $usuarios_ti->puesto ?? 'Puesto no especificado' }}</p>  <!-- CAMBIADO -->
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <p class="mt-1">{{ $usuarios_ti->usuario }}</p>  <!-- CAMBIADO -->
                                </div>
                                <div class="col-md-6 mb-3">
                                    <strong><i class="fas fa-id-card me-2"></i>Nombre Completo:</strong>
                                    <p class="mt-1">{{ $usuarios_ti->nombres }} {{ $usuarios_ti->apellidos }}</p>  <!-- CAMBIADO -->
                                </div>
                                <div class="col-md-6 mb-3">
                                    <strong><i class="fas fa-briefcase me-2"></i>Puesto:</strong>
                                    <p class="mt-1">{{ $usuarios_ti->puesto ?? 'No especificado' }}</p>  <!-- CAMBIADO -->
                                </div>
                                <div class="col-md-6 mb-3">
                                    <strong><i class="fas fa-phone me-2"></i>Teléfono:</strong>
                                    <p class="mt-1">{{ $usuarios_ti->telefono ?? 'No especificado' }}</p>  <!-- CAMBIADO -->
                                </div>
                                <div class="col-md-6 mb-3">
                                    <strong><i class="fas fa-shield-alt me-2"></i>Rol:</strong>
                                    <p class="mt-1">
                                        <span class="badge bg-{{ $usuarios_ti->rol == 'ADMIN' ? 'danger' : ($usuarios_ti->rol == 'AUXILIAR-TI' ? 'warning' : 'primary') }}">  <!-- CAMBIADO -->
                                            {{ $usuarios_ti->rol }}  <!-- CAMBIADO -->
                                        </span>
                                    </p>
                                </div>
                                <div class="col-md-6 mb-3">
                                    ...
                                    <p class="mt-1">{{ $usuarios_ti->created_at->format('d/m/Y H:i') }}</p>  <!-- CAMBIADO -->
                                </div>
                            </div>
                        </div>
                    </div>
