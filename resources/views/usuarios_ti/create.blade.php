    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center pb-2 mb-3 border-bottom">
                <h1 class="h2">
                    ...
                    @if($usuarios_ti->id === auth()->id())
                        Mi Perfil
                    @else
                        Editar Usuario: {{ $usuarios_ti->nombres }}
                    @endif
                </h1>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <!-- CORREGIDO: usar $usuarios_ti según tu ruta -->
                    <form action="{{ route('usuarios-ti.update', $usuarios_ti) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('usuario') is-invalid @enderror" 
                                       id="usuario" name="usuario" value="{{ old('usuario', $usuarios_ti->usuario) }}" required>
                                @error('usuario')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('rol') is-invalid @enderror" id="rol" name="rol" required
                                    {{ $usuarios_ti->id === auth()->id() ? 'disabled' : '' }}>
                                    <option value="">Seleccionar rol</option>
                                    <option value="ADMIN" {{ old('rol', $usuarios_ti->rol) == 'ADMIN' ? 'selected' : '' }}>Administrador</option>
                                    <option value="AUXILIAR-TI" {{ old('rol', $usuarios_ti->rol) == 'AUXILIAR-TI' ? 'selected' : '' }}>Auxiliar TI</option>
                                    <option value="PERSONAL-TI" {{ old('rol', $usuarios_ti->rol) == 'PERSONAL-TI' ? 'selected' : '' }}>Personal TI</option>
                                </select>
                                @if($usuarios_ti->id === auth()->id())
                                    <input type="hidden" name="rol" value="{{ $usuarios_ti->rol }}">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('nombres') is-invalid @enderror" 
                                       id="nombres" name="nombres" value="{{ old('nombres', $usuarios_ti->nombres) }}" required>
                                @error('nombres')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('apellidos') is-invalid @enderror" 
                                       id="apellidos" name="apellidos" value="{{ old('apellidos', $usuarios_ti->apellidos) }}">
                                @error('apellidos')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('puesto') is-invalid @enderror" 
                                       id="puesto" name="puesto" value="{{ old('puesto', $usuarios_ti->puesto) }}">
                                @error('puesto')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="tel" class="form-control @error('telefono') is-invalid @enderror" 
                                       id="telefono" name="telefono" value="{{ old('telefono', $usuarios_ti->telefono) }}">
                                @error('telefono')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
